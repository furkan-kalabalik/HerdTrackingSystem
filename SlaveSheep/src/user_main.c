...
void check_WIFIstate(void)
{
	...
	if(getstate == STATION_GOT_IP)//Check if sta is connected to ap
	{
		...
		struct ip_info info;
		const char remote_ip[4]={192,168,1,186};//tcp server ip address, the server must first use this ip to open
...
void tcp_client_init()//tcp cilent initialization
{
	wifi_set_opmode(0x01);//Set to station mode to save flash
	struct station_config stationconf;//stationa structure
	os_strcpy(stationconf.ssid,"ZyXEL_9511_plus");//Set the username of the incoming route
	os_strcpy(stationconf.password,"Furkan.1997");//Set the password to connect to the router
