...
typedef struct softuart_pin_t {
	...
	uint8_t gpio_func;
}  __attribute__((aligned(4))) softuart_pin_t;

typedef struct softuart_buffer_t {
	char __attribute__((aligned(4))) receive_buffer[SOFTUART_MAX_RX_BUFF]; 
	uint8_t receive_buffer_tail;
	uint8_t receive_buffer_head;
	uint8_t buffer_overflow; 
}  __attribute__((aligned(4))) softuart_buffer_t;
...
typedef struct {
	...
	uint16_t bit_time;
}  __attribute__((aligned(4))) Softuart;
...
typedef struct {
	uint32_t gpio_mux_name;
	uint8_t gpio_func;
}  __attribute__((aligned(4))) softuart_reg_t;
